...
export default function Nutrition() {
  ...
  const handleLogSubmit = async () => {
    // Only require member selection when there are multiple members
    if (!logForm.calories) return;
    if (members.length > 1 && !selectedMember) return;

    // Use selected member, or fall back to the first (and only) member
    ...
    try {
      await logNutrition(memberId, {
        ...
        water_intake_ml: parseInt(logForm.water_intake_ml) || 0,
      });
      setSaved(true);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Nutrition Analytics</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Weekly breakdown of your family's nutrient intake.</p>
        </div>
        {!isDemoMode && (
          <button
      ...
      {showLogForm && !isDemoMode && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-emerald-100 dark:border-emerald-900/40 p-6">
          <h3 className="font-semibold text-gray-900 dark:text-white mb-4">Log Today's Nutrition</h3>

          {members.length > 1 && (
            <div className="mb-4">
              <label className="block text-xs font-medium text-gray-400 mb-1">Family Member</label>
              <select value={selectedMember} onChange={e => setSelectedMember(e.target.value)}
                className="w-full max-w-xs px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            {[
              ...
              <div key={field}>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex gap-3">
            <button onClick={() => setShowLogForm(false)}
              className="px-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-600 dark:text-gray-400">
              Cancel
            </button>
            <button
              onClick={handleLogSubmit}
              disabled={isSaving || !logForm.calories || (members.length > 1 && !selectedMember)}
              className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-sm font-semibold rounded-xl flex items-center gap-2"
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map(stat => (
          <div key={stat.label} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-5">
            ...
            <div className={`text-xs font-medium ${stat.pos ? 'text-emerald-500' : 'text-rose-400'}`}>{stat.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6">
        <h2 className="font-semibold text-gray-900 dark:text-white mb-1">Weekly Macronutrients</h2>
        ...
        {isLoading ? (
          ...
          <div className="h-[260px] flex items-center justify-center text-sm text-gray-400 italic">No data yet. Use "Log Today" to record nutrition.</div>
        ) : (
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={displayChart} barGap={4}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
              <XAxis dataKey="day" tick={{ fontSize: 12, fill: '#9ca3af' }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 12, fill: '#9ca3af' }} axisLine={false} tickLine={false} width={35} />
              ...
              <Bar dataKey="fat" name="Fat (g)" fill="#10b981" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      {/* Calorie trend */}
      <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 p-6">
        <h2 className="font-semibold text-gray-900 dark:text-white mb-1">Calorie Trend</h2>
        ...
        {isLoading ? (
          ...
          <div className="h-[220px] flex items-center justify-center text-sm text-gray-400 italic">No calorie data yet.</div>
        ) : (
          <ResponsiveContainer width="100%" height={220}>
            <LineChart data={displayChart}>
              ...
              <Line type="monotone" dataKey="calories" stroke="#10b981" strokeWidth={2.5}
